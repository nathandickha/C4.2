...
// -----------------------------
  ...
  _setSectionOverlay(bounds, enabled, cutY = null, wallThickness = 0.2) {
    ...
    const addSpaSectionCaps = () => {
      ...
      // Main spa cap: keep the same footprint as the support/body, but extend
      // it upward to the spa floor top and downward by 200 mm so the visible
      // section cap closes the gaps above and below.
      if (spaSupportBox && !spaSupportBox.isEmpty()) {
        ...
        addProjectedCapFromBounds(mainCapBox, wallCapMatBase, 998, 0.007);
      }

      // Restore only the side seat caps, trimmed so they run down to the spa
      // floor top. Skip front/back seat boxes because those create the broad
      // centre band through the spa.
      if (spaFloorBox && !spaFloorBox.isEmpty() && spaSeatBoxes.length) {
        ...
        spaSeatBoxes.forEach((seatBox) => {
          ...
          const spanX = seatBox.max.x - seatBox.min.x;
          const spanY = seatBox.max.y - seatBox.min.y;
          ...
          addProjectedCapFromBounds(trimmed, wallCapMatBase, 998, 0.0075);
        });
      }
