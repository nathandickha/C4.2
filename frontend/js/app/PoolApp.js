...
// -----------------------------
  ...
  _setSectionOverlay(bounds, enabled, cutY = null, wallThickness = 0.2) {
    ...
    const addSectionBand = (x0, x3, includeWaterTint = true) => {
      ...
      const underfloorFillBottom = Math.min(
        ...topProfile.map((p) => p.y),
        resolvedLeftDepth,
        resolvedRightDepth,
        -Math.max(clampedDeep, clampedShallow, 1)
      ) - safeWallThickness - 2.0;
      const underfloorFillShape = new THREE.Shape([
        ...topProfile,
        new THREE.Vector2(x3Ext, underfloorFillBottom),
        new THREE.Vector2(x0Ext, underfloorFillBottom)
      ]);
      const underfloorFill = new THREE.Mesh(new THREE.ShapeGeometry(underfloorFillShape), floorMatBase.clone());
      underfloorFill.rotation.x = Math.PI * 0.5;
      underfloorFill.position.set(0, sectionY + 0.002, 0);
      underfloorFill.renderOrder = 995;
      group.add(underfloorFill);
