...
const SPA_BOTTOM_RAISE = 0.20; // raise spa bottom 200mm above pool deep-floor level
const SPA_SUPPORT_BOTTOM_RAISE = 0.20; // raise support/body a further 200mm above the spa bottom
...
function getSpaBottomZ(poolParams) {
  return getDeepFloorZ(poolParams) + SPA_BOTTOM_RAISE;
}
...
function updateSpaSeats(spa) {
  ...
  if (spaShape === 'circular') {
    ...
    if (seats.ring) {
      ...
      const seatGeo = createCircularRingGeometry(outerR, innerR, SPA_SEAT_THICKNESS);
...
function updateSpaWalls(spa) {
  ...
  const bottomZ=getSpaBottomZ(poolParams), topZ=getEffectiveSpaTopOffset(), h=topZ-bottomZ; spa.userData.height=h; spa.position.z=bottomZ+h/2;
  ...
  const floor=spa.userData.floor; const support=spa.userData.support; if(floor){ const floorHeight=0.2; let geo; if(spaShape==='circular') { geo=createCircularDiscGeometry(Math.max(0.05,radius-t),floorHeight); generateMeterUVsForCircularDiscGeometry(geo,tileSize); } else { geo=new THREE.BoxGeometry(l,w,floorHeight); generateMeterUVsForBoxGeometry(geo,tileSize);} floor.geometry.dispose(); floor.geometry=geo; floor.scale.set(1,1,1); const spaTopWorld=spa.position.z+spa.userData.height/2; const floorCenterZ=spaTopWorld-1-floorHeight/2; floor.position.set(0,0,floorCenterZ-spa.position.z);
    if (support) {
      ...
      const supportBottomWorldZ = bottomZ + SPA_SUPPORT_BOTTOM_RAISE;
      ...
      if (spaShape === 'circular') {
        ...
        // Stretch the spa floor/support mass down using the floor footprint so
        // the section reads as one continuous lower body and does not leave the
        // narrow side slits that appear when the support is inset from the floor.
...
spa.userData.spilloverMesh = spill;
  ...
  spa.position.z = getSpaBottomZ(poolParams) + ((getEffectiveSpaTopOffset() - getSpaBottomZ(poolParams)) * 0.5);
