...
export class EditablePolygon {
  ...
  rescaleTo(targetLength, targetWidth) {
    ...
    const deltaX = targetLength - currentLength;
    const deltaY = targetWidth - currentWidth;
    const halfDeltaX = deltaX * 0.5;
    const halfDeltaY = deltaY * 0.5;
    const eps = 1e-6;

    const shiftPoint = (pt) => {
      if (!pt) return;

      if (pt.x < centerX - eps) pt.x -= halfDeltaX;
      else if (pt.x > centerX + eps) pt.x += halfDeltaX;

      if (pt.y < centerY - eps) pt.y -= halfDeltaY;
      else if (pt.y > centerY + eps) pt.y += halfDeltaY;
    };

    this.vertices.forEach(shiftPoint);
    this.edges.forEach((edge) => {
      if (edge?.isCurved && edge?.control) shiftPoint(edge.control);
    });
