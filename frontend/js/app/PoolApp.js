...
setupPoolEditor() {
  ...
  _scheduleRebuildDebounced() {
    // During slider drags we only want the lightweight preview.
    // The accurate rebuild is forced on release in _setLiveDragging(false).
  ...
  _applyLivePreviewFromDirty() {
    ...
    if (footprintDirty || (isLShape && notchDirty)) {
      if (isLShape) {
        // Keep drag smooth: do not run accurate rebuilds mid-drag.
        // The committed rebuild happens on release. While dragging, the
        // existing pool geometry stays visible and the final topology catches
        // up when the slider is released.
    ...
    if (depthDirty) {
      ...
      if (useAccurateDepthRebuild) {
        // Keep depth drags lightweight as well; avoid accurate rebuilds until release.
        // We simply leave the committed geometry in place during drag for custom outlines.
        this.poolGroup.scale.set(1, 1, 1);
      } else {
