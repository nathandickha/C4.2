...
setupPoolEditor() {
  ...
  _applyLivePreviewFromDirty() {
    ...
    const isPolygon = this.isPolygonShape();

    // Never preview footprint edits by scaling the whole pool group for L-shape or
    // editable polygon/freeform pools. That stretches coping and can break curved edits.
    if (footprintDirty || (isLShape && notchDirty)) {
      if (isLShape || isPolygon) {
      ...
      this.poolGroup.scale.set(this.poolGroup.scale.x || 1, this.poolGroup.scale.y || 1, 1);
    }
