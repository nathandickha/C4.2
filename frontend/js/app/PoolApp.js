...
// -----------------------------
  ...
  _setSectionOverlay(bounds, enabled, cutY = null, wallThickness = 0.2) {
    ...
    const overlapsSectionY = (obj) => {
      if (!obj) return false;
      obj.updateMatrixWorld?.(true);
      this.poolGroup?.updateMatrixWorld?.(true);
      const bb = new THREE.Box3().setFromObject(obj);
      if (bb.isEmpty()) return false;
      const worldMinY = bb.min.y;
      const worldMaxY = bb.max.y;
      return sectionY >= worldMinY - 1e-4 && sectionY <= worldMaxY + 1e-4;
    };

    const getLocalXRange = (obj) => {
      if (!obj) return null;
      obj.updateMatrixWorld?.(true);
      this.poolGroup?.updateMatrixWorld?.(true);
      const bb = new THREE.Box3().setFromObject(obj);
      if (bb.isEmpty()) return null;
      const inv = new THREE.Matrix4().copy(this.poolGroup.matrixWorld).invert();
      const pts = [
      ...
      if (!Number.isFinite(minX) || !Number.isFinite(maxX) || maxX - minX < 1e-4) return null;
      return [minX, maxX];
    };

    const subtractRanges = (baseIntervals, blockers) => {
      let out = baseIntervals.slice();
      for (const blocker of blockers) {
        if (!blocker || blocker.length < 2) continue;
        const b0 = blocker[0];
        const b1 = blocker[1];
        const next = [];
        for (const seg of out) {
          const s0 = seg[0];
          const s1 = seg[1];
          if (b1 <= s0 + 1e-4 || b0 >= s1 - 1e-4) {
            next.push(seg);
            continue;
          }
          if (b0 > s0 + 1e-4) next.push([s0, Math.max(s0, b0)]);
          if (b1 < s1 - 1e-4) next.push([Math.min(s1, b1), s1]);
        }
        out = next.filter((seg) => seg[1] - seg[0] > 0.02);
      }
      return out;
    };
    ...
    const blockerRanges = [];
    const spaLikeRoots = [this.spa, this.ground?.userData?.spaChannelGroup].filter(Boolean);
    spaLikeRoots.forEach((obj) => {
      if (!overlapsSectionY(obj)) return;
      const range = getLocalXRange(obj);
      if (!range) return;
      blockerRanges.push([range[0] - safeWallThickness - 0.02, range[1] + safeWallThickness + 0.02]);
    });

    const effectiveIntervals = blockerRanges.length ? subtractRanges(intervals, blockerRanges) : intervals.slice();

    const floorMatBase = makeBasicMat(0x6ea7c8, 0.95);
    const wallCapMatBase = makeBasicMat(0x6ea7c8, 0.95);
    ...
    const isNearAny = (value, values, tol = 0.03) => values.some((v) => Math.abs(v - value) <= tol);

    const originalBoundaryXs = intervals.flat();
    const blockerBoundaryXs = blockerRanges.flat();

    const addSectionBand = (x0, x3, includeWaterTint = true, opts = {}) => {
      const drawLeftBoundaryCap = opts.drawLeftBoundaryCap !== false;
      const drawRightBoundaryCap = opts.drawRightBoundaryCap !== false;
      ...
      if (drawLeftBoundaryCap) {
        ...
        copingLeftCap.position.set(0, sectionY + 0.005, 0);
        copingLeftCap.renderOrder = 998;
        group.add(copingLeftCap);
      }

      if (drawRightBoundaryCap) {
        ...
        copingRightCap.position.set(0, sectionY + 0.005, 0);
    ...
    if (effectiveIntervals.length) {
      effectiveIntervals.forEach(([x0, x3]) => {
        const leftIsOriginalBoundary = isNearAny(x0, originalBoundaryXs) && !isNearAny(x0, blockerBoundaryXs, 0.04);
        const rightIsOriginalBoundary = isNearAny(x3, originalBoundaryXs) && !isNearAny(x3, blockerBoundaryXs, 0.04);
        addSectionBand(x0, x3, true, {
          drawLeftBoundaryCap: leftIsOriginalBoundary,
          drawRightBoundaryCap: rightIsOriginalBoundary,
        });
      });
      const minIntervalX = Math.min(...effectiveIntervals.map((p) => p[0]));
      const maxIntervalX = Math.max(...effectiveIntervals.map((p) => p[1]));
