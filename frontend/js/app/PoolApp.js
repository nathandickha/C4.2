...
// -----------------------------
  ...
  _setSectionOverlay(bounds, enabled, cutY = null, wallThickness = 0.2) {
    ...
    const addSpaSectionCaps = () => {
      ...
      // Main spa cap: use the support/body footprint, but extend it 200 mm
      // further downward so the main section cap reaches lower.
      ...
      // Restore the side seat caps and trim them so they run only from the
      // seat cut down to the spa floor top.
      if (spaFloorBox && !spaFloorBox.isEmpty() && spaSeatBoxes.length) {
        ...
        spaSeatBoxes.forEach((seatBox) => {
          ...
          // Only restore the side seat caps. Front/back seat boxes create the
          // broad centre band through the spa opening.
