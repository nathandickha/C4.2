...
export class EditablePolygon {
  ...
  rescaleTo(targetLength, targetWidth) {
    ...
    // --- Freeform/Oval/Custom mode: bounding-box scale, but preserve
    // curved-corner radii so global length/width changes do not distort
    // bespoke curved walls.
    ...
    const curvedEdgeRadii = this.edges.map((edge, index) => {
      if (!edge?.isCurved || !edge.control || !this.vertices[index]) return null;
      const next = this.vertices[this.nextIndex(index)];
      if (!next) return null;
      return {
        startRadius: edge.control.distanceTo(this.vertices[index]),
        endRadius: edge.control.distanceTo(next)
      };
    });
    ...
    this.edges.forEach((edge) => {
      ...
      edge.control.x = centerX + (edge.control.x - centerX) * scaleX;
      edge.control.y = centerY + (edge.control.y - centerY) * scaleY;
    });

    this.edges.forEach((edge, index) => {
      if (!edge?.isCurved || !edge.control || !this.vertices[index]) return;
      const nextIndex = this.nextIndex(index);
      const nextVertex = this.vertices[nextIndex];
      const radii = curvedEdgeRadii[index];
      if (!nextVertex || !radii) return;

      const startDir = this.vertices[index].clone().sub(edge.control);
      const endDir = nextVertex.clone().sub(edge.control);
      if (startDir.lengthSq() <= 1e-10 || endDir.lengthSq() <= 1e-10) return;

      startDir.normalize();
      endDir.normalize();

      this.vertices[index].copy(edge.control).addScaledVector(startDir, Math.max(0, radii.startRadius));
      nextVertex.copy(edge.control).addScaledVector(endDir, Math.max(0, radii.endRadius));
    });
