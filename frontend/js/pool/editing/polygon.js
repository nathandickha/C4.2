...
export class EditablePolygon {
  ...
  rescaleTo(targetLength, targetWidth) {
    ...
    // --- Freeform/Oval/Custom mode: resize by pushing the outer sides in/out
    // instead of globally scaling the entire polygon. This keeps coping widths
    // and curved-corner radii stable while still letting overall length/width
    // edits work after a curve has been added.
    ...
    const halfDeltaX = (targetLength - currentLength) * 0.5;
    const halfDeltaY = (targetWidth - currentWidth) * 0.5;
    const epsX = Math.max(1e-5, currentLength * 1e-5);
    const epsY = Math.max(1e-5, currentWidth * 1e-5);

    const shiftCoord = (value, center, halfDelta, eps) => {
      if (Math.abs(value - center) <= eps) return value;
      return value < center ? value - halfDelta : value + halfDelta;
    };

    this.vertices.forEach((v) => {
      v.x = shiftCoord(v.x, centerX, halfDeltaX, epsX);
      v.y = shiftCoord(v.y, centerY, halfDeltaY, epsY);
    });

    this.edges.forEach((edge) => {
      if (!edge?.isCurved || !edge.control) return;
      edge.control.x = shiftCoord(edge.control.x, centerX, halfDeltaX, epsX);
      edge.control.y = shiftCoord(edge.control.y, centerY, halfDeltaY, epsY);
    });
