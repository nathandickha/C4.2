...
setupPoolEditor() {
  ...
  _updateFootprintGuideFromParams(baseParams, currentParams) {
    ...
    const targetLength = Math.max(0.01, Number((currentParams?.length ?? currentSize.x) || 0.01));
    const targetWidth  = Math.max(0.01, Number((currentParams?.width  ?? currentSize.y) || 0.01));

    const hx = targetLength * 0.5;
    const hy = targetWidth * 0.5;
    const z = Math.max(box.max.z + 0.03, 0.03);
  ...
  _applyLivePreviewFromDirty() {
    ...
    // Length/width changes should rebuild actual geometry for every pool shape.
    // Whole-group X/Y scaling stretches coping and rounded geometry during drag,
    // which is exactly the behaviour we want to avoid across the entire project.
    const useAccurateFootprintPreview = true;

    if (footprintDirty && !useAccurateFootprintPreview) {
    ...
    if (footprintDirty || (isLShape && notchDirty)) {
      if (useAccurateFootprintPreview) {
        // Show a lightweight target-footprint guide immediately so slider motion feels smooth,
        // while the real geometry continues to rebuild on a throttled path.
    ...
    if (!footprintDirty && !this._live.dragging) {
      this._hideFootprintGuide();
    }
