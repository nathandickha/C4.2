  ...
  _setSectionFrontShellHidden(centerY, hidden) {
    ...
    roots.forEach((root) => {
      root.traverse((obj) => {
        ...
        // Only hide front/back-oriented shell strips here.
        // End walls are thin in X, while front/shared spa walls are thin in Y.
  ...
  _setSectionOverlay(bounds, enabled, cutY = null, wallThickness = 0.2) {
    ...
    const addSpaSectionCaps = () => {
      ...
      const addProjectedCapFromBounds = (box, matBase, renderOrder = 997, yOffset = 0.006) => {
        ...
        cap.position.set(0, sectionY + yOffset, 0);
      ...
      spa.traverse((obj) => {
        ...
        if (obj.userData?.isSpaFloor) {
          spaFloorBox = addProjectedCapFromMesh(obj, floorMatBase, 997, 0.006) || spaFloorBox;
          return;
        }
        if (obj.userData?.isSpaSeat) {
          addProjectedCapFromMesh(obj, wallCapMatBase, 998, 0.007);
          return;
        }
      ...
      if (spaFloorBox) {
        ...
        spaWallBoxes.forEach((box) => {
          if (!box || box.isEmpty()) return;
          wallBounds.expandByPoint(box.min);
          wallBounds.expandByPoint(box.max);
          hasWallBounds = true;
        });

        const baseBottomZ = hasWallBounds ? wallBounds.min.z : spaFloorBox.min.z;
        const baseTopZ = spaFloorBox.min.z;
        if ((baseTopZ - baseBottomZ) > 1e-4) {
          const underFloorBox = new THREE.Box3(
            new THREE.Vector3(spaFloorBox.min.x, sectionY - 0.01, baseBottomZ),
            new THREE.Vector3(spaFloorBox.max.x, sectionY + 0.01, baseTopZ)
          );
          addProjectedCapFromBounds(underFloorBox, wallCapMatBase, 996, 0.005);
        }
      }
    };
