...
export class PoolApp {
    ...
    this._live = {
      ...
      baseParams: null,
      // lightweight footprint guide shown during length/width drags so motion feels responsive
      footprintGuide: null,
      footprintGuideVisible: false
    };
  }
...
// -----------------------------
  ...
  _setSectionFrontShellHidden(centerY, hidden) {
    const roots = [this.poolGroup, this.spa, this.ground?.userData?.spaChannelGroup].filter(Boolean);
    roots.forEach((root) => {
      root.traverse((obj) => {
        if (!obj?.isMesh) return;
        const isSectionShell = !!(
  ...
  _setSectionOverlay(bounds, enabled, cutY = null, wallThickness = 0.2) {
    ...
    if (!enabled || !bounds || bounds.isEmpty()) return;
    ...
    const isRect = this.poolParams?.shape === 'rectangular' && this.poolGroup;
    if (isRect) {
      const length = Math.max(0.1, this.poolParams.length || size.x || 1);
      ...
      const halfL = length * 0.5;
      const stepFoot = this.poolGroup?.userData?.stepFootprintLen || 0;
      const originX = this.poolGroup?.userData?.originX || (-halfL + stepFoot);
      const fullLen = halfL - originX;
      ...
      const x2 = halfL - dFlat;
      const x3 = halfL;
      const zShallow = -clampedShallow;
      const zDeep = -clampedDeep;
      const slabExtend = 0.2;
      const x0Ext = x0 - slabExtend;
      const x3Ext = x3 + slabExtend;

      const topProfile = [
        new THREE.Vector2(x0Ext, zShallow),
        new THREE.Vector2(x1, zShallow),
        new THREE.Vector2(x2, zDeep),
        new THREE.Vector2(x3Ext, zDeep)
      ];
      ...
      const waterTintGeo = new THREE.ShapeGeometry(waterTintShape);
      const waterTintMat = new THREE.MeshBasicMaterial({
        color: 0x9cc6dc,
        transparent: true,
        opacity: 0.12,
      ...
      const waterTint = new THREE.Mesh(waterTintGeo, waterTintMat);
      waterTint.rotation.x = Math.PI * 0.5;
      waterTint.position.set(0, sectionY + 0.001, 0);
      waterTint.renderOrder = 994;
      group.add(waterTint);

      const underFloorMaskBottom = Math.min(zShallow, zDeep) - wallThickness - Math.max(clampedDeep, 1) - 2;
      this.sectionUnderfloorClipConfig = {
        cutY: sectionY,
        halfWidth: Math.max(0.15, (sectionFloorWidth || size.y || 0.6) * 0.5 + 0.35),
        minX: x0 - wallThickness - slabExtend - 0.05,
        maxX: x3 + wallThickness + slabExtend + 0.05,
        x1,
        x2,
        topZ0: zShallow - wallThickness + 0.01,
        topZ1: zShallow - wallThickness + 0.01,
        topZ2: zDeep - wallThickness + 0.01,
      ...
      const profile = [
        ...topProfile,
        new THREE.Vector2(x3, zDeep - wallThickness),
        new THREE.Vector2(x2, zDeep - wallThickness),
        new THREE.Vector2(x1, zShallow - wallThickness),
        new THREE.Vector2(x0, zShallow - wallThickness)
      ];
      const floorShape = new THREE.Shape(profile);
      ...
      const floorCap = new THREE.Mesh(floorGeo, floorMat);
      floorCap.rotation.x = Math.PI * 0.5;
      floorCap.position.set(0, sectionY + 0.003, 0);
      floorCap.renderOrder = 996;
      floorMat.depthTest = false;
      floorMat.depthWrite = false;
      group.add(floorCap);

      const wallCapMat = new THREE.MeshBasicMaterial({
        color: 0x6ea7c8,
        transparent: true,
        opacity: 0.95,
        side: THREE.DoubleSide,
      ...
      const leftWallShape = new THREE.Shape([
        new THREE.Vector2(x0 - wallThickness, 0),
        new THREE.Vector2(x0, 0),
        new THREE.Vector2(x0, zShallow - wallThickness),
        new THREE.Vector2(x0 - wallThickness, zShallow - wallThickness)
      ]);
      const leftWallCap = new THREE.Mesh(new THREE.ShapeGeometry(leftWallShape), wallCapMat.clone());
      leftWallCap.rotation.x = Math.PI * 0.5;
      leftWallCap.position.set(0, sectionY + 0.004, 0);
      leftWallCap.renderOrder = 997;
      group.add(leftWallCap);

      const rightWallShape = new THREE.Shape([
        new THREE.Vector2(x3, 0),
        new THREE.Vector2(x3 + wallThickness, 0),
        new THREE.Vector2(x3 + wallThickness, zDeep - wallThickness),
        new THREE.Vector2(x3, zDeep - wallThickness)
      ]);
      const rightWallCap = new THREE.Mesh(new THREE.ShapeGeometry(rightWallShape), wallCapMat.clone());
      rightWallCap.rotation.x = Math.PI * 0.5;
      rightWallCap.position.set(0, sectionY + 0.004, 0);
      rightWallCap.renderOrder = 997;
      group.add(rightWallCap);

      const copingSegments = this.poolGroup?.userData?.copingSegments || null;
      const copingCapMat = new THREE.MeshBasicMaterial({
        color: 0xe3ddd2,
        transparent: true,
        opacity: 0.98,
        side: THREE.DoubleSide,
      ...
      copingCapDefs.forEach((copingMesh) => {
        copingMesh.updateMatrixWorld?.(true);
        const copingBounds = new THREE.Box3().setFromObject(copingMesh);
        if (copingBounds.isEmpty()) return;
        const minX = copingBounds.min.x - this.poolGroup.position.x;
        const maxX = copingBounds.max.x - this.poolGroup.position.x;
        const minZ = copingBounds.min.z - this.poolGroup.position.z;
        const maxZ = copingBounds.max.z - this.poolGroup.position.z;
        if (![minX, maxX, minZ, maxZ].every(Number.isFinite)) return;
        const copingCapShape = new THREE.Shape([
          new THREE.Vector2(minX, minZ),
          new THREE.Vector2(maxX, minZ),
          new THREE.Vector2(maxX, maxZ),
          new THREE.Vector2(minX, maxZ)
        ]);
        const copingCap = new THREE.Mesh(new THREE.ShapeGeometry(copingCapShape), copingCapMat.clone());
        copingCap.rotation.x = Math.PI * 0.5;
        copingCap.position.set(0, sectionY + 0.005, 0);
        copingCap.renderOrder = 998;
        group.add(copingCap);
      });
    }
  ...
  setSectionViewEnabled(enabled) {
    ...
    if (enabled === this.sectionViewEnabled) return;
    ...
    if (enabled) {
      ...
      const wallThickness = Math.max(0.05, this.poolGroup?.userData?.wallThickness || 0.2);
      const sectionCutY = center.y;
      waters.forEach((root) => this._setSectionWaterClip(root, sectionCutY));
      const shellPlane = new THREE.Plane(new THREE.Vector3(0, 1, 0), -sectionCutY);
      [this.poolGroup, this.spa, this.ground?.userData?.spaChannelGroup].filter(Boolean).forEach((root) => this._setSectionShellClip(root, shellPlane));
  ...
  _refreshSectionViewPresentation() {
    ...
    const bounds = new THREE.Box3();
    bounds.expandByObject(this.poolGroup);
    if (this.spa) bounds.expandByObject(this.spa);
    if (bounds.isEmpty()) return;
  ...
  async rebuildPoolForCurrentShape() {
    ...
    if (this.isPolygonShape()) {
      if (!this.editablePolygon) {
        ...
        Number.isFinite(this.poolParams.length) &&
        Number.isFinite(this.poolParams.width) &&
        typeof this.editablePolygon.rescaleTo === "function"
      ) {
        // Keep the editable polygon footprint in sync with the length/width sliders.
        // Once a customised curve exists, the rebuilt mesh comes from editablePolygon,
        // not from poolParams directly, so the polygon itself must be resized first.
...
updateGroundVoid(this.ground || this.scene.userData.ground, this.poolGroup, this.spa);
    ...
    if (!(this._live?.dragging && (this._live?.dirty?.has("length") || this._live?.dirty?.has("width")))) {
      this._hideFootprintGuide();
    }
...
setupPoolEditor() {
  ...
  _ensureFootprintGuide() {
    if (this._live.footprintGuide) return this._live.footprintGuide;
    ...
    guide.name = 'PoolFootprintGuide';
    guide.renderOrder = 1000;
    guide.visible = false;
    guide.frustumCulled = false;
    this.scene?.add?.(guide);
    this._live.footprintGuide = guide;
    return guide;
  }

  _hideFootprintGuide() {
    const guide = this._live?.footprintGuide;
    if (!guide) return;
    guide.visible = false;
    this._live.footprintGuideVisible = false;
  }

  _updateFootprintGuideFromParams(baseParams, currentParams) {
    if (!this.scene || !this.poolGroup) return;

    const guide = this._ensureFootprintGuide();
    const box = new THREE.Box3().setFromObject(this.poolGroup);
    if (!Number.isFinite(box.min.x) || !Number.isFinite(box.max.x)) return;

    const targetLength = Math.max(0.01, Number((currentParams?.length ?? baseParams?.length ?? this.poolParams.length) || 0.01));
    const targetWidth  = Math.max(0.01, Number((currentParams?.width  ?? baseParams?.width  ?? this.poolParams.width ) || 0.01));
    const baseLength   = Math.max(0.01, Number((baseParams?.length ?? this.poolParams.length) || 0.01));
    const baseWidth    = Math.max(0.01, Number((baseParams?.width  ?? this.poolParams.width ) || 0.01));
    const sx = targetLength / baseLength;
    const sy = targetWidth / baseWidth;
    const zWorld = Math.max(box.max.z + 0.03, 0.03);

    const localPts = Array.isArray(this.poolGroup?.userData?.outerPts) && this.poolGroup.userData.outerPts.length >= 3
    ...
    if (localPts) {
      ...
      const worldPts = [];
      for (const p of localPts) {
        const lx = cx + (p.x - cx) * sx;
        const ly = cy + (p.y - cy) * sy;
        const wp = this.poolGroup.localToWorld(new THREE.Vector3(lx, ly, zWorld));
        worldPts.push(wp);
      }
      if (worldPts.length >= 2) {
        const first = worldPts[0];
        worldPts.push(first.clone());
        verts = new Float32Array(worldPts.length * 3);
    ...
    if (!verts) {
      ...
      const hx = targetLength * 0.5;
      const hy = targetWidth * 0.5;
      verts = new Float32Array([
        center.x - hx, center.y - hy, zWorld,
        center.x + hx, center.y - hy, zWorld,
        center.x + hx, center.y + hy, zWorld,
        center.x - hx, center.y + hy, zWorld,
        center.x - hx, center.y - hy, zWorld,
    ...
    guide.geometry.setAttribute('position', new THREE.BufferAttribute(verts, 3));
    guide.geometry.computeBoundingSphere();
    guide.visible = true;
    this._live.footprintGuideVisible = true;
  }
  ...
  _applyLivePreviewFromDirty() {
    ...
    // Never scale the full pool group for length/width preview.
    // Keep the real model static during drag and show only a lightweight ghost outline.
    try { this.poolGroup.scale.set(1, 1, 1); } catch (_) {}

    if (footprintDirty) {
      this._updateFootprintGuideFromParams(base, p);
    } else if (!this._live.dragging) {
      this._hideFootprintGuide();
    }
    ...
    // Keep footprint dirty until release so the accurate rebuild runs once on commit.
