  ...
  _applyLivePreviewFromDirty() {
    ...
    if (footprintDirty || (isLShape && notchDirty)) {
      if (useAccurateFootprintPreview) {
        // Editable polygon footprints and L-shapes must rebuild from geometry data.
        // Whole-group scale does not update the underlying polygon, so sliders appear
        // to stop working once a customised curve has been applied.
