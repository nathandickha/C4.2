    ...
    const addSpaSectionCaps = () => {
      ...
      spa.traverse((obj) => {
        ...
        if (obj.userData?.isSpaFloor) {
          // Keep track of the spa floor bounds if needed later, but do not add
          // a projected floor cap. The support/base now carries the lower mass,
          // and a separate floor projection reads as a false centre cap.
        ...
        if (obj.userData?.isSpaSeat) {
          // Seat meshes are mitered ring segments. Projecting their world-space
          // bounding boxes onto the section plane fabricates a broad cross-bar
          // through the spa. Leave the clipped seat geometry visible instead of
          // adding a rectangular overlay cap here.
          return;
        }
